...
<div class="cards">
  <div class="card"><div class="label">Total revenue</div><div class="value" id="m-rev">–</div><div class="hint" id="m-revsub"></div></div>
...
async function refresh() {
  ...
  document.getElementById("m-rev").textContent = num(d.revenue_atomic);
  document.getElementById("m-revsub").textContent = "USDC · " + d.challenges + " quotes · " + d.rejected + " rejected";
  ...
  document.getElementById("m-todaysub").textContent = "USDC · last 24 hours";
  ...
  tp.innerHTML = d.top_payers.length
    ? "<table><tr><th>agent (wallet)</th><th>calls</th><th>revenue</th></tr>" +
      d.top_payers.map(r =>
        `<tr><td class="addr">${esc(short(r.payer))}</td><td>${r.calls}</td><td>${usdc(r.revenue)}</td></tr>`).join("") +
      "</table>"
